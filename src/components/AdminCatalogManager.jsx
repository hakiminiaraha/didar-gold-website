  ...
  return (
    <section>
      <div className="mb-6 flex flex-col justify-between gap-4 md:flex-row md:items-end">
        <div><p className="text-xs tracking-[0.16em] text-[var(--gold-text)]">DIDAR CATALOG</p><h2 className="mt-2 text-3xl">محصولات و کالکشن‌ها</h2><p className="mt-2 text-sm text-[#041E42]/55">ساخت، ویرایش، انتشار و مرتب‌سازی کاتالوگ بدون تغییر کد</p></div>
      ...
      <div className="grid gap-6 xl:grid-cols-[340px_1fr]">
        <aside className="border border-[#B08A57]/25 bg-white">
          <div className="flex items-center justify-between border-b border-[#B08A57]/20 px-4 py-4"><h3>{type === "product" ? "فهرست محصولات" : "فهرست کالکشن‌ها"}</h3><button type="button" onClick={newItem} className="inline-flex items-center gap-1 text-xs text-[var(--gold-text)]"><Plus size={15} /> رکورد جدید</button></div>
          <div className="max-h-[720px] overflow-y-auto">{loading ? <div className="grid h-40 place-items-center"><LoaderCircle className="animate-spin text-[var(--gold-text)]" /></div> : items.map((item) => <button key={item.id} type="button" onClick={() => editItem(item)} className={`flex w-full items-center gap-3 border-b border-[#B08A57]/10 p-3 text-right ${form.id === item.id ? "bg-[#041E42] text-white" : "hover:bg-[#F7F3EE]"}`}><span className="h-14 w-14 shrink-0 overflow-hidden bg-[#F7F3EE]">{(item.data.image || item.data.hero) ? <img src={item.data.image || item.data.hero} alt="" className="h-full w-full object-cover" /> : <ImagePlus size={18} className="m-auto mt-4 text-[var(--gold-text)]" />}</span><span className="min-w-0 flex-1"><strong className="block truncate text-sm font-normal">{item.data.name?.fa || item.slug}</strong><span className={`mt-1 block text-[10px] ${form.id === item.id ? "text-white/45" : "text-[#041E42]/40"}`}>{item.slug} · {item.status === "published" ? "منتشرشده" : "پیش‌نویس"}</span></span><span className="text-xs text-[var(--gold-text)]">{Number(item.sortOrder).toLocaleString("fa-IR")}</span></button>)}</div>
        </aside>
...
function MediaField({ label, value, onChange, onUpload }) {
  return <div><TextField label={label} value={value} onChange={onChange} dir="ltr" /><button type="button" onClick={onUpload} className="mt-2 inline-flex items-center gap-2 text-[11px] text-[var(--gold-text)]"><Upload size={13} /> آپلود از دستگاه</button></div>;
}
