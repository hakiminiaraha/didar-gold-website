...
export default function AdminInboxManager() {
  ...
  return <section>
    <div className="mb-6"><p className="text-xs tracking-[0.16em] text-[var(--gold-text)]">DIDAR RELATIONSHIPS</p><h2 className="mt-2 text-3xl">درخواست‌های تماس و مشاوره</h2><p className="mt-2 text-sm text-[#041E42]/55">پیگیری متمرکز گفت‌وگوهای ورودی از سایت</p></div>
    ...
    <div className="grid min-h-[680px] overflow-hidden border border-[#B08A57]/25 bg-white xl:grid-cols-[430px_1fr]">
      ...
      <div className="p-5 md:p-8">{selected ? <><div className="flex flex-col justify-between gap-4 border-b border-[#B08A57]/20 pb-6 md:flex-row md:items-start"><div><p className="text-xs text-[var(--gold-text)]" dir="ltr">{selected.referenceCode}</p><h3 className="mt-2 text-3xl">{selected.fullName}</h3><p className="mt-2 text-sm text-[#041E42]/50">{reasonLabels[selected.reason]} · {formatDate(selected.createdAt)}</p></div><select value={selected.status} onChange={(event) => updateSelected("status", event.target.value)} className="h-11 border border-[#B08A57]/30 bg-white px-4 text-sm"><option value="new">جدید</option><option value="contacted">تماس گرفته‌شده</option><option value="scheduled">زمان‌بندی‌شده</option><option value="closed">بسته</option></select></div>
        ...
        {notice && <p className="mt-4 text-xs text-[var(--gold-text)]">{notice}</p>}<button type="button" onClick={save} disabled={saving} className="mt-5 inline-flex h-11 items-center gap-2 bg-[#B08A57] px-6 text-sm text-white disabled:opacity-50">{saving ? <LoaderCircle size={16} className="animate-spin" /> : <Save size={16} />} ذخیره پیگیری</button></> : <div className="grid min-h-[560px] place-items-center text-center"><div><CheckCircle2 size={34} className="mx-auto text-[var(--gold-text)]" /><p className="mt-4 text-sm text-[#041E42]/45">برای مشاهده جزئیات، یک درخواست را انتخاب کن.</p></div></div>}</div>
    </div>
  </section>;
}

function Info({ icon: Icon, label, value, ltr = false }) {
  return <div className="flex gap-3 border border-[#B08A57]/20 p-4"><Icon size={17} className="mt-0.5 shrink-0 text-[var(--gold-text)]" /><div><p className="text-[10px] text-[#041E42]/40">{label}</p><p className="mt-1 text-sm" dir={ltr ? "ltr" : "rtl"}>{value}</p></div></div>;
}
