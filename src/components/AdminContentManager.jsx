  ...
  return (
    <section>
      <div className="mb-6 flex flex-col justify-between gap-4 xl:flex-row xl:items-end">
        <div><p className="text-xs tracking-[0.16em] text-[var(--gold-text)]">DIDAR CONTENT STUDIO</p><h2 className="mt-2 text-3xl">مدیریت محتوای سایت</h2><p className="mt-2 text-sm text-[#041E42]/55">روی هر بخش در پیش‌نمایش کلیک کن، ویرایش کن و منتشر کن.</p></div>
      ...
      <div className="grid min-h-[720px] overflow-hidden border border-[#B08A57]/25 bg-white xl:grid-cols-[380px_1fr]">
        <aside className="flex min-h-0 flex-col border-b border-[#B08A57]/20 xl:border-b-0 xl:border-l">
          <div className="border-b border-[#B08A57]/20 p-4">
            <label className="flex h-11 items-center gap-3 border border-[#B08A57]/25 px-3"><Search size={16} className="text-[var(--gold-text)]" /><input value={query} onChange={(event) => setQuery(event.target.value)} placeholder="جست‌وجوی متن یا رسانه" className="w-full bg-transparent text-sm outline-none" /></label>
          </div>
          <div className="max-h-[290px] overflow-y-auto border-b border-[#B08A57]/20 xl:max-h-[340px]">
            {loading && <div className="grid h-40 place-items-center"><LoaderCircle className="animate-spin text-[var(--gold-text)]" /></div>}
            {!loading && filtered.map((entry) => {
              const id = getEntryId(entry); const Icon = entry.contentType === "text" ? FileText : entry.contentType === "video" ? Video : ImagePlus;
              return <button key={id} type="button" onClick={() => setSelectedId(id)} className={`flex w-full gap-3 border-b border-[#B08A57]/10 px-4 py-3 text-right ${selectedId === id ? "bg-[#041E42] text-white" : "hover:bg-[#F7F3EE]"}`}><Icon size={16} className="mt-1 shrink-0 text-[var(--gold-text)]" /><span className="min-w-0"><span className="block truncate text-xs">{entry.label}</span><span className={`mt-1 block text-[10px] ${selectedId === id ? "text-white/45" : "text-[#041E42]/40"}`}>{typeLabels[entry.contentType]}</span></span></button>;
            })}
          </div>
          <div className="flex-1 p-4">
            {selected ? <>
              <div className="flex items-center justify-between"><p className="text-xs text-[#041E42]/55">{selected.label}</p><span className="text-[10px] text-[var(--gold-text)]">{typeLabels[selected.contentType]}</span></div>
