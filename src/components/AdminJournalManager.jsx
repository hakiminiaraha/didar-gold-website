...
export default function AdminJournalManager() {
  ...
  return <section>
    <div className="mb-6 flex flex-col justify-between gap-4 md:flex-row md:items-end"><div><p className="text-xs tracking-[0.16em] text-[var(--gold-text)]">DIDAR EDITORIAL</p><h2 className="mt-2 text-3xl">مدیریت ژورنال</h2><p className="mt-2 text-sm text-[#041E42]/55">نوشتن، ویرایش و انتشار روایت‌های دیدار</p></div><button type="button" onClick={() => { setForm(clone(emptyArticle)); setNotice(""); }} className="inline-flex h-11 items-center gap-2 border border-[#B08A57]/30 bg-white px-5 text-sm"><Plus size={16} /> مقاله جدید</button></div>
    <div className="grid gap-6 xl:grid-cols-[340px_1fr]">
      <aside className="border border-[#B08A57]/25 bg-white"><div className="border-b border-[#B08A57]/20 px-4 py-4"><h3>مقاله‌ها</h3></div><div className="max-h-[760px] overflow-y-auto">{loading ? <div className="grid h-40 place-items-center"><LoaderCircle className="animate-spin text-[var(--gold-text)]" /></div> : articles.map((article) => <button key={article.id} type="button" onClick={() => { setForm(clone(article)); setNotice(""); }} className={`flex w-full items-center gap-3 border-b border-[#B08A57]/10 p-3 text-right ${form.id === article.id ? "bg-[#041E42] text-white" : "hover:bg-[#F7F3EE]"}`}><span className="h-14 w-14 shrink-0 overflow-hidden bg-[#F7F3EE]">{article.image ? <img src={article.image} alt="" className="h-full w-full object-cover" /> : <BookOpen size={18} className="m-auto mt-4 text-[var(--gold-text)]" />}</span><span className="min-w-0"><strong className="block truncate text-sm font-normal">{article.content.fa.title || article.slug}</strong><span className={`mt-1 block text-[10px] ${form.id === article.id ? "text-white/45" : "text-[#041E42]/40"}`}>{article.status === "published" ? "منتشرشده" : "پیش‌نویس"} · {article.category}</span></span></button>)}</div></aside>
      <form onSubmit={save} className="border border-[#B08A57]/25 bg-white">
        ...
        <div className="grid gap-5 p-5 md:grid-cols-2 md:p-7">
          ...
          <div className="md:col-span-2"><Field label="تصویر شاخص" value={form.image} onChange={(value) => setForm((current) => ({ ...current, image: value }))} dir="ltr" /><button type="button" onClick={() => fileInput.current?.click()} className="mt-2 inline-flex items-center gap-2 text-[11px] text-[var(--gold-text)]"><Upload size={13} /> آپلود تصویر</button></div>
