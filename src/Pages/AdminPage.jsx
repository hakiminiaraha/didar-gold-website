        ...
        <div className="mx-auto max-w-[1400px]">
          ...
          {loading ? <div className="grid min-h-[60vh] place-items-center"><span className="h-10 w-10 animate-spin rounded-full border border-[#B08A57]/25 border-t-[#B08A57]" /></div> : <>
            ...
            {activeTab === "overview" && <section>
              <div className="mb-7"><p className="text-xs tracking-[0.16em] text-[#B08A57]">DIDAR OPERATIONS</p><h2 className="mt-2 text-3xl">تصویر امروز پلتفرم</h2><p className="mt-2 text-sm text-[#041E42]/55">شاخص‌های عضویت، تعامل و امنیت در یک نگاه</p></div>
            ...
            {activeTab === "users" && <section>
              ...
              <label className="mb-5 flex max-w-md items-center gap-3 border border-[#B08A57]/30 bg-white px-4"><Search size={17} className="text-[#B08A57]" /><input value={query} onChange={(event) => setQuery(event.target.value)} placeholder="جست‌وجو در شماره موبایل" className="h-12 w-full bg-transparent text-sm outline-none" /></label>
              ...
              {selectedUser && <section className="mt-7 border border-[#B08A57]/25 bg-white">
                <div className="flex flex-col gap-4 border-b border-[#B08A57]/20 px-5 py-5 sm:flex-row sm:items-center sm:justify-between">
                  <div><p className="text-xs text-[#B08A57]">نشست‌های ورود</p><h3 className="mt-1 text-xl" dir="ltr">{selectedUser.mobileMasked}</h3></div>
                ...
                {sessionsLoading ? <p className="px-5 py-10 text-center text-sm text-[#041E42]/45">در حال دریافت نشست‌ها...</p> : sessions.length ? <div className="divide-y divide-[#B08A57]/15">{sessions.map((session) => <div key={session.id} className="flex items-start gap-4 px-5 py-5"><MonitorSmartphone size={21} className="mt-0.5 shrink-0 text-[#B08A57]" /><div className="min-w-0"><p className="truncate text-sm" dir="ltr">{session.userAgent}</p><p className="mt-2 text-xs text-[#041E42]/45">آخرین فعالیت: {formatDate(session.lastSeenAt)} · انقضا: {formatDate(session.expiresAt)}</p></div></div>)}</div> : <p className="px-5 py-10 text-center text-sm text-[#041E42]/45">نشست فعالی برای این کاربر وجود ندارد.</p>}
              </section>}
            </section>}
...
function StatusRow({ icon: Icon, title, value, active = false }) {
  return <div className="flex items-center justify-between border-b border-[#B08A57]/15 px-5 py-5 last:border-b-0 sm:px-7"><div className="flex items-center gap-4"><Icon size={21} strokeWidth={1.3} className="text-[#B08A57]" /><span className="text-sm sm:text-base">{title}</span></div><span className={`inline-flex items-center gap-2 text-xs ${active ? "text-emerald-700" : "text-[#041E42]/55"}`}><i className={`h-1.5 w-1.5 rounded-full ${active ? "bg-emerald-500" : "bg-[#B08A57]"}`} />{value}</span></div>;
}
