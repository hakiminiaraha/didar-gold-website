...
// List view only needs each locale's title + excerpt (cards), not the full article
// body (which can be ~500 KB each). Trim content so the list response stays small;
// the per-slug fetch still returns the full article via serializeArticle.
function serializeArticleSummary(row) {
  const content = JSON.parse(row.content_json);
  const trim = (locale) => (locale && typeof locale === "object" ? { title: locale.title, excerpt: locale.excerpt } : locale);
  return {
    id: String(row.id), slug: row.slug, status: row.status, sortOrder: row.sort_order,
    featured: Boolean(row.featured), category: row.category, pillar: row.pillar, image: row.image,
    dateLabel: row.date_label, readLabel: JSON.parse(row.read_label_json),
    content: { fa: trim(content.fa), en: trim(content.en) },
    createdAt: row.created_at, updatedAt: row.updated_at,
  };
}
...
export async function publicJournal({ response, url }) {
  ...
  const articles = (await db.prepare("SELECT * FROM journal_articles WHERE status = 'published' ORDER BY featured DESC, sort_order, created_at").all()).map(serializeArticleSummary);
...
export async function createArticle({ request, response }) {
  ...
  try {
    ...
    recordAudit({ userId: user.id, eventType: "journal.article_created", targetType: "article", targetId: payload.slug });
    sendJson(response, 201, serializeArticle(article));
  } catch (error) {
    if (isUniqueViolation(error)) throw new HttpError(409, "ARTICLE_SLUG_EXISTS");
    throw error;
  }
}
...
export async function updateArticle({ request, response }) {
  ...
  try {
    ...
    recordAudit({ userId: user.id, eventType: "journal.article_updated", targetType: "article", targetId: payload.slug, metadata: { status: payload.status } });
...
export async function deleteArticle({ request, response }) {
  ...
  recordAudit({ userId: user.id, eventType: "journal.article_deleted", targetType: "article", targetId: article.slug });
